...
{$I ./../../src/ConfigAbstractMem.inc }

type
   TestTAbstractStorage = class(TTestCase)
   strict private
   private
     procedure InitializeStorage(ASender : TAbstractStorage; const ACurrentName, ANewName : String; ACurrentVersion, AUpdateToVersion : Integer);
...
begin
  ...
  ambt_index := TAbstractMemBTreeDataIndex<Integer>.Create(ambt_int,ASender.StorageStructInformation.GetStorageStructAMZone('TAbstractMemBTreeDataAbstract<Integer>'),False,5,TComparison_Integer);
  try
    Assert(aml_1.Count>1);
    Assert(aml_int.Count>1);
    Assert(aml_int_ord.Count>1);
    Assert(ambt_1.Count>1);
    Assert(ambt_int.Count>1);
    Assert(ambt_index.Count>1);
    // Destroy some
    ambt_1.EraseTree;
  finally
    aml_1.Free;
...
procedure TestTAbstractStorage.InitializeStorage(ASender: TAbstractStorage; const ACurrentName, ANewName : String; ACurrentVersion, AUpdateToVersion: Integer);
var amz : TAMZone;
  aml_1 : TAbstractMemTList;
  aml_int : TAbstractMemTList_TTestRecord;
  aml_int_ord : TAbstractMemOrderedTList_TTestRecord;
...
begin
  ...
  Las := TAbstractStorage.Create(ClassName+'.ams',False,'TEST',1,InitializeStorage);
  try
